...
\keywords{Asynchronous Binary Automata Networks \and Local Causality Graph \and Heuristics.}
\end{abstract}
...
%Over-approximation&True&True&False&False\\
%\hline
%Under-approximation&True&False&True&False\\
%\hline
%Reachability&True&Inconclusive&N/A&False\\
%\end{tabular}
%\caption{Truth table of LCG analysis}
%\label{tab:1}
%\end{table}
...
\label{fig:3}
\end{figure}
%\item $\mathbf{\Sigma}=\{a,b,c\}$, $\mathbf{T}=\{\acm{b_0}{a_0}{a_1},\acm{a_0}{b_0}{b_1},\acm{\varnothing}{a_1}{a_0}, \acm{\varnothing}{b_1}{b_0},\acm{\{a_1,b_1\}}{c_0}{c_1}\}$, $\omega=c_1$. 
...
%We propose a method to find a trajectory starting from the initial state and ending with desired final state.
%
%\begin{theorem}
%In an ABAN, partial reachability and the reachability obtained by SLCG are equivalent, if there is no cycle nor conflict.
%\end{theorem}
%\begin{proof}
...
%Although all the local states in $s$ are reachable, the existence of conflicts leads to potential unreachability of $s$.
...
%Example: in Fig \ref{fig:3}, to reach $\{a_1,b_1\}$, $a_1\triangleright b_1\triangleright a_1$, so the joint state is unreachable. 
...
One possible counterexample is shown in Fig. \ref{FigConflictInForks}. %Sufficiency is proved. Hence $reach (s)=\bigvee reach (Perm (s))$.
\qed\end{proof}
